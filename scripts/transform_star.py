...
def run_etl_transformation():
    ...
    # 3. Cập nhật dim_time (Dùng published_at để vẽ Trend thời gian thực)
    cursor.execute("""
    ...
    # 4. CẬP NHẬT BẢNG FACT (Đưa dữ liệu về Dashboard)
    # Dùng REPLACE INTO để đè dữ liệu cũ, tránh bị nhân đôi (Duplicate)
    sql_fact = """
    REPLACE INTO fact_sentiments (comment_id, author_key, video_key, time_key, sentiment_label, confidence_score)
    SELECT sc.comment_id, sc.author_name, sc.video_id, DATE_FORMAT(sc.published_at, '%Y%m%d%H'), sr.label, sr.confidence_score
